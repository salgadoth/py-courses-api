...
@app.post("/courses/chapter/rate/{course_id}/{chapter_id}")
def post_chapter_rating(course_id: str, chapter_id: str, rating: int = Query(..., gt=-2, lt=2)):
    ...
    try:
        ...
        raise HTTPException(status_code = 404, detail='Chapter not found, check provided id.') from e
    
    try:
        chapter['rating']['total'] += rating
        chapter['rating']['total'] += 1
    except KeyError:
